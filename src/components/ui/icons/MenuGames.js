import { IconBG } from './IconBG';
import Icon from './icon'

export const MenuGames = ({ fill, width, height }) => {
  return (
    <IconBG  h='48px' w='48px' bg='#191C23' r='40px' margin='10px 18px' padding='0px'>
      <Icon margin='12px 12px 0' width={width || '48px'} height={height || '48px'} fill={fill}>
        <rect x="13" y="14" width="2" height="2" rx="1" fill="#7D869E"/>
        <path d="M10 14H6" stroke="#7D869E" stroke-linecap="round"/>
        <path d="M8 12L8 16" stroke="#7D869E" stroke-linecap="round"/>
        <rect x="16" y="12" width="2" height="2" rx="1" fill="#7D869E"/>
        <path d="M14 8V8C14 7.58326 14 7.37488 13.9655 7.19144C13.8455 6.5546 13.4245 6.01534 12.8358 5.74455C12.6662 5.66654 12.464 5.616 12.0597 5.51493L12 5.5C11.5388 5.3847 11.3082 5.32706 11.1171 5.233C10.5686 4.96315 10.1737 4.45731 10.0449 3.85979C10 3.65151 10 3.41382 10 2.93845V2" stroke="#7D869E" stroke-linecap="round"/>
        <path d="M3 14C3 11.4412 3 10.1618 3.61994 9.28042C3.77954 9.05351 3.96572 8.85041 4.17372 8.6763C4.98164 8 6.15442 8 8.5 8H15.5C17.8456 8 19.0184 8 19.8263 8.6763C20.0343 8.85041 20.2205 9.05351 20.3801 9.28042C21 10.1618 21 11.4412 21 14C21 16.5588 21 17.8382 20.3801 18.7196C20.2205 18.9465 20.0343 19.1496 19.8263 19.3237C19.0184 20 17.8456 20 15.5 20H8.5C6.15442 20 4.98164 20 4.17372 19.3237C3.96572 19.1496 3.77954 18.9465 3.61994 18.7196C3 17.8382 3 16.5588 3 14Z" stroke="#7D869E"/>
      </Icon>
    </IconBG>
  );
}